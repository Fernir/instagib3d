import { english_words as words } from './dict.js';

class NickGenerator {
  constructor(max_prefix) {
    function Node(ch) {
      let self = this;
      this.ch = ch;
      this.count = 0;
      ...
      this.getRandomChild = function () {
        ...
        let char;
        while (counts > 0) {
          let id = (Math.random() * chars.length) | 0;
          char = chars[id];
      ...
      this.getRandomCommonChild = function (node) {
        ...
        for (let prop in self) {
          if (self[prop] instanceof Node && node[prop] !== undefined) {
            let count = self[prop].count * node[prop].count;
    ...
    let subword = new Tree();
    words.forEach(function (word) {
      for (let i = 0; i < word.length - MAX_LENGTH + 1; i++) {
    ...
    this.gener = function (length) {
      let nick = '';
      let prefix_node = prefix.head;

      let only_prefix_length = Math.max(Math.min(MAX_LENGTH, length - MAX_LENGTH), 0);
      ...
      function subwordNode() {
        let last = nick.slice(-MAX_LENGTH + 1);
        let ret = subword.head;
      ...
      for (
        let i = Math.min(MAX_LENGTH, length);
        i < Math.min(length - MAX_LENGTH + 1, length);
        i++
      ) {
