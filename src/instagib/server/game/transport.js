...
let ALFABET = " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,!?-+\\/():;_|%=[]><  абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЧЦШЩЪЫЬЭЮЯ";
...
function getString(view, offset)
{
    ...
    let str = "";
    let length = view.getUint8(offset);   offset++;
    for (let i = 0; i < length; i++)
    {
        let char = view.getUint8(offset); offset++;
        str += decode(char);
    }
...
function getBot(view, offset, bot, isCamera)
{
    ...
    let state = view.getUint8(offset); offset++;
    ...
    let seria = view.getUint8(offset); offset++;
    bot.seria = seria - 128;
    ...
    if (isCamera)
    {
        ...
        let weamask = view.getUint32(offset); offset += 4;
        ...
        let statsmask = view.getUint8(offset); offset++;
...
function getEvent(view, offset, event)
{
    ...
    else
    {
        ...
        if (event.type === EVENT.PAIN || event.type === EVENT.BOT_DEAD)
        {
            let dx = toFloat(view.getInt16(offset), 50 * 256); offset += 2;
            let dy = toFloat(view.getInt16(offset), 50 * 256); offset += 2;
            event.dir = new Vector(dx, dy);
            event.botid = view.getUint16(offset); offset += 2;
        }
        else if (event.type === EVENT.BULLET_RESPAWN)
        {
            let val = view.getUint8(offset); offset++;
        ...
        else if (event.type === EVENT.LINE_SHOOT)
        {
            ...
            event.power = (val >> 4) & 0xf;
            event.angle = toFloat(view.getUint16(offset)); offset += 2;
            let size = view.getUint8(offset); offset++;
            ...
            if (event.bullet_type === WEAPON.SHAFT)
            {
                ...
                let normx = toFloat(view.getInt16(offset)); offset += 2;
                let normy = toFloat(view.getInt16(offset)); offset += 2;
                event.norm_dir = new Vector(normx, normy);
                let napx = toFloat(view.getInt16(offset)); offset += 2;
                let napy = toFloat(view.getInt16(offset)); offset += 2;
...
{
    let ret = {};
    let count = view.getUint8(offset); offset++;
    for (let i = 0; i < count; i++)
    {
        let id = view.getUint16(offset); offset += 2;
...
constructor(socket, game)
{
    ...
    function onData(data)
    {
        ...
        switch (cmd)
        {
            ...
            case SV_FRAME: {
                let offset = 1;
                const server_time = view.getUint32(offset);    offset += 4;
                const mybot = new ServerBot();
                offset = getBot(view, offset, mybot, true);

                const count_bots = view.getUint8(offset); offset++;
                const listbots = [];
                ...
                const count_items = view.getUint8(offset); offset++;
                const listitems = [];
                ...
                const count_events = view.getUint8(offset); offset++;
                const listevents = [];
                ...
                const count_rows = view.getUint8(offset); offset++;
                const table = [];
