...
gameplay._E = function (killer_scores, deaded_scores) {
  return (
    gameplay.ratingkoef /
    (1 + Math.pow(10.0, (killer_scores - deaded_scores) / gameplay.ratingdiap))
  );
};
...
Event.on('botdead', function (bot, killer, bullet, _isLava) {
  ...
  if (bot === killer) {
    ...
    let updelta = gameplay._E(killer.stats.scores, 1200);
    killer.stats.scores += updelta * koef_killer;
    let downdelta = gameplay._E(1200, bot.stats.scores);
