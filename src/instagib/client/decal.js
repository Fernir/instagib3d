...
constructor(size_class)
{
    ...
    let shader_blit = new Shader(vert, frag_blit,
    [
        "mat_pos", "tex",
    ]);
    let shader_lerp = new Shader(vert, frag_lerp,
    [
        "mat_pos", "tex", "tex_dest", "color",
    ]);
    let shader_add = new Shader(vert, frag_add,
    [
        "mat_pos", "tex", "tex_dest", "color",
    ]);
    let shader_drying = new Shader(vert, frag_drying,
    [
        "mat_pos", "tex",
    ]);
    ...
    const size_quad = 32;
    let fbo_decals = new Array(size * size);
    let fbo_decal = new Framebuffer(512, 512);
    let fbo_dry = new Framebuffer(512, 512);
    let current_dry_fbo = 0;
    ...
    function render_base(fbo, shader, texId, dynent, secondId, color)
    {
        ...
        state.mat4.scal(mat, [1 / size_quad, 1 / size_quad]);
        
        let mb = state.mat4.create();
        state.mat4.trans(mb, [2 * (dynent.pos.x - size_quad / 2), 2 * (-dynent.pos.y + size_quad / 2)]);
        
        let mr = state.mat4.create();
        state.mat4.rotateZ(mr, mr, dynent.angle);
        state.mat4.mul(mb, mb, mr);
        state.mat4.mul(mat, mat, mb);
        
        state.mat4.scal(mat, dynent.size.toVec());
        shader.matrix(shader.mat_pos, mat);
        state.gl.drawArrays(state.gl.TRIANGLE_STRIP, 0, 4);
        fbo.unbind();
        state.stats.count_decal++;
    }
    function render_decal_absolute_coord(fbo, dynent, tex, color, sh_add)
    {
        let pos = Vector.sub2(dynent.pos, fbo.x, fbo.y);
        let rad = dynent.size.length() * 0.5;
        ...
        let blend = state.gl.isEnabled(state.gl.BLEND);
        if (blend)
        {
        ...
        render_base(fbo_decal, shader_blit, fbo.fbo.getTexture(), localDynent);
        render_base(fbo.fbo, sh_add ? shader_add : shader_lerp, tex.getId(), localDynent, fbo_decal.getTexture(), color);
    ...
    function drying(fbo)
    {
        ...
        {
            let drying_time = Console.variable("drying-time", "time of drying decals", 1000);
            fbo.time = Date.now() + parseInt(drying_time);
            let dest = fbo_dry;
            dest.bind();
            shader_drying.use();
            shader_drying.matrix(shader_drying.mat_pos, state.mat4.create());
    ...
    this.render = function(camera)
    {
        ...
        fbo_decals.forEach(function(fbo)
        {
            let pos = new Vector(fbo.x + size_quad / 2, fbo.y + size_quad / 2);
            Dynent.render(camera, fbo.fbo.getTexture(), state.LevelRender.shader_simple, pos, [size_quad, size_quad], 0, 
            {
...
Decal.render_decal = function(dynent, tex, color, sh_add = false)
{
    ...
    {
        state.gameClient.getLevelRender().getDecal().render_decal(dynent, tex, color, sh_add);
    }
}

Event.on("cl_botdead", function(pos, dir, id)
{
    let color = state.Bot.isMutant(id) ? [0, 0.5, 0, 1] : [0.5, 0, 0, 1];
    Decal.render_decal(
    {
        pos: pos,
        size: new Vector(3, 3),
        angle: Math.random() * Math.PI * 2,
    }, state.Particle.splash_textures[20], color);
});

Event.on("cl_bulletlinecollide", function(bullet, dest, norm_dir)
{
    let radius = [0.4, 1, 0.4];
    Decal.render_decal(
    {
        pos: dest,
        angle: Math.random() * Math.PI * 2,
        size: new Vector(radius[bullet.type], radius[bullet.type]),
    }, state.Weapon.tex_decal, [0.1, 0.1, 0.1, 1], bullet.type === WEAPON.SHAFT);
});
