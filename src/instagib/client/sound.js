...
class Sound {
  ...
  play(pos) {
    if (!state.soundEnabled || !state.audioUnlocked || state.Howler.ctx?.state === 'suspended') return null;
...
Event.on('cl_takeweapon', function (pos) { state.Item.snd_weapon.play(pos); });
Event.on('cl_takehealth', function (pos) { state.Item.snd_health.play(pos); });
Event.on('cl_takeshield', function (pos) { state.Item.snd_health.play(pos); });
Event.on('cl_takepower',  function (pos) { state.Item.snd_power.play(pos); });
Event.on('cl_itemrespawn', function (pos) { state.Item.snd_respawn.play(pos); });
