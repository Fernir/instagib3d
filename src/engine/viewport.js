...
function visualViewportKeyboardOpen() {
...
export function shouldTrackVisualViewport() {
  if (!state.mobileControls) return false;
  if (state.Console?.show) return true;
  if (typeof window === 'undefined') return false;
  const vv = window.visualViewport;
  if (!vv) return false;
  if (visualViewportKeyboardOpen()) return true;
 ...
 */
export function applyViewportLayout(canvas) {
  if (!canvas || typeof window === 'undefined') return;

  const vv = window.visualViewport;
  const trackVv = shouldTrackVisualViewport();
