...
function pushGameEvent(target, type, pos, dir, arg1, arg2) {
  const events = target?.game?.events;
  if (!events || !pos) return;
  events.push(new GameEvent(type, pos, dir, arg1, arg2));
}

Event.on('botrespawn', function (bot) {
  pushGameEvent(bot, EVENT.BOT_RESPAWN, bot.dynent?.pos, null, bot.id);
});
...
Event.on('botpain', function (bot, bullet, opponent) {
  if (bullet && bullet.type !== WEAPON.RAIL) {
    ...
    if (!pos) return;
    pushGameEvent(bot, EVENT.PAIN, pos, Vector.mul(unit, mag), bot.id);
  }
});
...
Event.on('botdead', function (bot, killer, bullet) {
  ...
  pushGameEvent(bot, EVENT.BOT_DEAD, bot.dynent?.pos, dir, bot.id);
});

Event.on('takeweapon', function (bot) {
  pushGameEvent(bot, EVENT.TAKE_WEAPON, bot.dynent?.pos, null);
});

Event.on('takehealth', function (bot) {
  pushGameEvent(bot, EVENT.TAKE_HEALTH, bot.dynent?.pos, null);
});

Event.on('takeshield', function (bot) {
  pushGameEvent(bot, EVENT.TAKE_SHIELD, bot.dynent?.pos, null);
});

Event.on('takepower', function (bot) {
  pushGameEvent(bot, EVENT.TAKE_POWER, bot.dynent?.pos, null);
});

Event.on('itemrespawn', function (item) {
  pushGameEvent(item, EVENT.ITEM_RESPAWN, item.dynent.pos, null);
});

Event.on('bulletdead', function (bullet) {
  pushGameEvent(bullet.owner, EVENT.BULLET_DEAD, bullet.dynent.pos, null, bullet);
});

Event.on('lineshoot', function (bullet) {
  pushGameEvent(bullet.owner, EVENT.LINE_SHOOT, bullet.dynent.pos, null, bullet);
});

Event.on('bulletrespawn', function (bullet, sound) {
  pushGameEvent(bullet.owner, EVENT.BULLET_RESPAWN, bullet.dynent.pos, null, bullet, sound);
});
