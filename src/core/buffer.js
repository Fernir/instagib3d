...
class Buffer {
  constructor(size, seed) {
      ...
      do {
        ...
        for (let j = 0; j < size; j++) {
          for (let i = 0; i < size; i++) {
            let x = ((i * freq) / size) | 0;
            let y = ((j * freq) / size) | 0;
            let tx = (i * freq) / size - x;
            let ty = (j * freq) / size - y;
            let x1 = buf_data(x, y);
            let old_x = x;
            x++;
            if (x > freq - 1) x = 0;
            let x2 = buf_data(x, y);
            let xx = cos_lerp(x1, x2, tx);

            y++;
            if (y > freq - 1) y = 0;
            let y1 = buf_data(old_x, y);
            let y2 = buf_data(x, y);
            let yy = cos_lerp(y1, y2, tx);
            let h = cos_lerp(xx, yy, ty);
    ...
    this.normalize = function (a, b) {
      ...
      let k = (b - a) / (max - min);
      for (let i = 0; i < size * size; i++) data[i] = (data[i] - min) * k + a;
    ...
    this.gaussian_fast = function (radius, src_buf, dir, mask) {
      ...
      for (let j = 0; j < size; j++) {
        for (let i = 0; i < size; i++) {
          if (mask && mask.getData(i, j) < 0.5) continue;
    ...
    this.getGaussian = function (radius, mask) {
      let blur_x = new Buffer(size);
      let blur = new Buffer(size);
    ...
    */
      ...
      do {
        ...
        for (let j = 1; j < size - 1; j++) {
          for (let i = 1; i < size - 1; i++) {
            let a00 = self.getData(i, j) > 0.5;
            let a10 = self.getData(i + 1, j) > 0.5;
            let a01 = self.getData(i, j + 1) > 0.5;
            let a11 = self.getData(i + 1, j + 1) > 0.5;
            ...
            let a0_1 = self.getData(i, j - 1) > 0.5;
            let a_10 = self.getData(i - 1, j) > 0.5;
    ...
    this.fill_isolated_area = function () {
      let time = Date.now();
      let filled = new Array(size * size);
      for (let i = 0; i < size * size; i++) filled[i] = data[i] > 0.5 ? 255 : 0;

      function fill(id, coord_x, coord_y) {
        let coords = [{ x: coord_x, y: coord_y }];
      ...
      for (let i = 0; i < size * size; i++) {
        let val = filled[i];
        if (val !== 255 && val !== index_with_max_count) data[i] = 1;
      }
    ...
    this.draw = function (src_image) {
      let time = Date.now();
      let koef = src_image.getSize() / size;
      for (let j = 0; j < size; j++) {
        let y = (j * koef) | 0;
    ...
    this.copy = function (src_image) {
      ...
      for (let i = 0; i < size * size; i++) data[i] = src_image.getData(i);
    ...
    this.generate_river = function (amplitude, count_river) {
      ...
      let rnd = rand.next();
      let begin = { pos: new Vector(rnd * size, size - 1), next: null };
      let end = { pos: new Vector((1 - rnd) * size, 0), next: null };

      // 2) split algorithm
      let all_river = [];

      function split(a, b, out) {
        let dir = Vector.sub(b.pos, a.pos);
        let length = dir.length();
        if (length < 5) return;

        let c = { pos: null, next: null };
      ...
      function random_point() {
        let ret = { pos: null, next: null };
        let a = rand.next() * size;
        let b = rand.next() < 0.5 ? 0 : size - 1;
      ...
      for (let count = 0; count < count_river; count++) {
        let pivot_id = (rand.next() * all_river.length) | 0;
        let pivot = all_river[pivot_id];
        let point_begin = { pos: new Vector(pivot.pos), next: null };
        let point_end = random_point();
        let sub_river = [point_begin];
      ...
      function raster_river(riv, val) {
        for (let i = 0; i < riv.length - 1; i++) {
          ...
          if (riv[i].next) raster_river(riv[i].next, val * 0.5);
        }
      }
    ...
    this.shadow = function (src_buf, sun, length) {
      ...
      length = length || 12;
      let dir = Vector.normalize(sun);

      for (let j = 0; j < size; j++) {
        for (let i = 0; i < size; i++) {
          let shadow_value = 0.0;
          let count = 0;
          let x0 = i;
          let y0 = j;
          for (let k = 0; k < length; k++) {
            let x = x0 + dir.x * k; //coord trace in levelmap
            let y = y0 + dir.y * k;
            if (x < 0 || y < 0 || x > size - 1 || y > size - 1) continue;

            let val = src_buf.getData(x | 0, y | 0);
            shadow_value += val;
            count++;
          }
